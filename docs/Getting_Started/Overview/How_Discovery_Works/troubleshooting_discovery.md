...
The below topics will be describing the Global Settings however users can make
adjustments to specific Snapshots. Making changes to the Global Settings and
then testing them in a previous snapshot may not apply your changes. Here are
two methods for testing settings.
...
- Make your changes globally and then run a new Snapshot. You could also filter
  the allowed discovery scope in the Snapshot specific Settings to a subset of
  `/32` addresses to speed up discovery. In the example below I am telling
  discovery only to test 1 device.
...
These are self-defined IP Addresses that IP Fabric will try to connect to and
...
If you have remote locations possibly through a WAN that is not owned by you,
this would be the location to put the IP Addresses of some of your devices so IP
Fabric can discover and crawl that site.
...
The Discovery History table is also used by IP Fabric as the seed addresses.
...
discovers a neighbor in this network it will try to log in and discover it. If
the IP address is not in the include or is in the exclude list then it will not
try to connect and discover the device.

The exclude list is a great way to exclude networks managed by a different
department or perhaps vendors or devices that IPF does not currently support.
For instance a customer had discovery hanging due to it trying to log into a
terminal server which IPF could not understand; once added to the exclude list
...
- `Trace` signifies using traceroute to RFC1918 addresses to help discover your
  internal network. This is helpful for the VM as IPF might not be able to log
...
Please see [SSH/TELNET](../../../IP_Fabric_Settings/advanced/SSH_telnet.md) for
...
This is the best table to start as not only do you see the local host and
interface names but also the remote hostname, interface name, and remote IP.
Since devices can connect to multiple other devices it is possible of duplicate
...
This table shows you unmanaged neighbors based many protocols and has a built-in
...
Perhaps you would like to audit IP Fabric (or vice versa) using an NMS. This can
...
traffic is being blocked by an ACL or Firewall. This is why it is recommend
...
option is to use a jumphost and documentation can be found
...
If you find other errors you can refer to this
document [Connectivity report - Type of Error](common_problems/connectivity_report.md)
and the CLI output of the device is usually helpful as well.

If you need further assistance please feel free to reach out to your Systems
Engineer or open a ticket following
