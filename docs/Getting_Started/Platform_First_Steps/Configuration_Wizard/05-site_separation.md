...
he dropdown is intuitive and will let you search based on SN or hostname. Currently there is an issue where IP Fabric will not search for devices discovered via an API in the UI. Even though it appears no devices match the SN it will still perform the site separation correctly on the next snapshot.
...
a) If SNMP Location matches “IPFABRIC, (LAB01)” → Site LAB01
b) If Hostname matches “^L21” → Site MPLS
c) If Hostname matches “^(L\d{1,2})” → Site L2-99
3. **Try to assign devices without sites based on device neighborship** (if enabled)
a) https://ipfabric.atlassian.net/wiki/spaces/ND/pages/2887647267/Site+Separation+from+4.3#Device-Neighborship