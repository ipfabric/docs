...
| | `status`               | string  | `done` -- Loaded.<br/>`unloaded` -- Unloaded.<br/>`ready` -- The snapshot record exists, but discovery has not started yet.<br/>`run` -- Discovery or snapshot load is in progress.<br/>`finishing` -- Almost done. Tasker is creating topology and other calculations like Site Separation.<br/>`error` -- Errored.<br/>`canceled` -- Discovery, Snapshot Load, or Topology Calculation was canceled. The snapshot stays loaded with partial data. See [Canceled Snapshots](../../IP_Fabric_GUI/discovery_snapshot.md#canceled-snapshots). |
| `finishStatus`         | string  | `done` -- Done.<br/>`error` -- Errored.<br/>`canceled` -- Canceled.<br/>'' -- Other.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `isLastSnapshot`       | Boolean | `true` if this is the last loaded snapshot (`$last`). |
