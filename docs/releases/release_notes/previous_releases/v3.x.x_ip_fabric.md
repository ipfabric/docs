...
- Check Point Gaia - fixed API endpoint /show-gateways-and-servers
  errors - VSX clusters & checkpoint host objects
- Check Point Gaia - fixed parsing of bandwidth information for command
...
- Added support for Check Point Gaia Embedded platform
...
- Check Point Gaia - error about missing router ID was shown even if
  BGP was disabled
...
- Check Point Gaia - fixed processing of zone firewall services
  (DCE-RPC, port ranges)
...
- Check Point Gaia - added support for VSX firewalls and switches

### Improvements

- Check Point Gaia - use the default Router ID for BGP if it isn’t
...
- Arista EOS - fixed parsing of different output for command `show ip
route vrf \<vrfName>`
- Check Point Gaia - fixed mapping of different output for API endpoint
...
- Logging in to the platform was slow - rewritten DB query for GET
  /snapshot endpoint
...
- Check Point API data wasn't download on add new devices to the
  snapshot - fixed
- Check Point Gaia - fixed parsing of different output for command
  `show ospf neighbors detailed`
- Check Point Gaia - run command `show security-gateway memory
...
- First Boot Wizard - Self-signed certificate with one letter
...
- Check Point Gaia - add cluster virtual IP addresses to the interfaces
- Check Point Gaia - fixed parsing of different output for command
  `show ospf neighbors detailed`
...
- Device Explorer - Detailed view for each device
  - When you click hostname value in every table then you can see
...
- Cisco WLC Air - fixed parsing of different output for command `show
lag summary`
- Check Point Gaia - fixed parsing of different output for command `show
...
- Check Point Gaia - improved OS version detection, support for new
  versions outputs.
...
- Cisco NX-OS - fixed Nexus 5000 interface MTU was wrong due to Cisco
  bug.
- Cisco WLC - fixed parsing of different output for `show
run-config` commands
...
- Added BGP support for Check Point Gaia, Cisco ASA, Extreme XOS,
...
- First Boot Wizard - Fixed - NTP configuration hangs.
...
Please post any ideas for improvement to `https://ideas.ipfabric.io` (obsolete)
and in case of any problems, contact our support team, either via the
