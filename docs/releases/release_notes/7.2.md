...
After an upgrade  a `Something went wrong` error when accessing the IP Fabric GUI after upgrading was shown. This issue has been resolved on `7.2.17` release.
...
#### Configuration management process not closing SSH sessions

On HP Aruba SW family devices, the Configuration Management process doesn't close SSH connections.
In case a login banner is not detected, SSH sessions may remain open.


To check the entire situation, go to the IPF appliance CLI and run the following command:

```bash
netstat -tnpa | grep 'ESTABLISHED' | grep syslogWorkerBu | grep 22
```

If SSH connections remain open, you will see the following:
...
tcp        0      0 10.0.192.108:5650       10.20.130.22:22         ESTABLISHED 624/syslogWorkerBui
...
To resolve this problem, please proceed with restarting the Configuration Management service using:
