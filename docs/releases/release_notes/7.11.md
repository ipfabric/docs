...
This version has been **withdrawn** and must not be installed. It introduced a
regression that prevents loading previously created snapshots. A corrected
build will be published in the next patch release.

Customers who have already deployed `v7.11.8` and are experiencing issues
should [contact Support](https://ipfabric.atlassian.net/servicedesk/customer/portal/1/group/1/create/1).
Where a pre-upgrade VM snapshot is available, Support can assist
with rolling back to the last stable release, `v7.11.7`.
