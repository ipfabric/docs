...
This version has been **withdrawn** and is **no longer available for
download or installation**. It may cause issues with loading previously
created snapshots in some environments. Many existing deployments continue to
run `v7.11.8` without impact. A corrected build will be published in the next
patch release.

Customers who deployed `v7.11.8` and experience snapshot load issues should [contact Support](https://ipfabric.atlassian.net/servicedesk/customer/portal/1/group/1/create/1).
If a pre-upgrade VM snapshot is available, Support can assist with rolling back to the last stable release, `v7.11.7`.
