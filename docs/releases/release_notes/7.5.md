...
- The following column operations are no more supported:
  - Operations (cross-column) on text and text array column: `like`, `notlike`, `reg`, `nreg`, `ireg`, `nireg`
  - Operations (cross-column) on IP and IPv6 column: `like`, `notlike`, `reg`, `nreg`
  - Operations (cross-column) on routing and IPv6 routing column: `like`, `notlike`, `reg`, `nreg`
  - Operations on columns referencing external entities: `ieq`, `nieq`, `like`, `notlike`, `reg`, `ireg`, `nreg`, `nireg`
