...
### Broken PSQL Since Release 7.3  

The **PostgreSQL (PSQL)** database was introduced in release 7.3, but it was not yet actively used. In some deployments, the PSQL database may be broken due to a missing role: `ipf_appliance_shared_role`.

**Before upgrading to version 7.5**, ensure that the previous upgrade to **7.3 completed without the error**: `ERROR: role "ipf_appliance_shared_role" does not exist`. Run the following command to check:  

```
grep "ipf_appliance_shared_role" /var/log/ipf/system-upgrade-ipf/package-upgrade
```

!!! Warning

    If the error found, do **not** proceed with the upgrade.
...
### Missing PSQL Database Status

The status page lacks PSQL database status information and a service restart option.
...
- Snapshots from Release `5.X` and earlier are not supported and may not load properly