...
| **Benchmark (optional but strongly recommended)** | Run `ipf-checker` for **at least 30 minutes** to measure disk performance. (The test saturates the appliance; **schedule during off-hours**.) |
...
??? note "Step 2 - Prepare the Appliance"
    1. Shut down the appliance gracefully from the UI (**System → Shutdown**).
    2. Create a cold snapshot in your hypervisor; verify its completion.
    3. Power the appliance back on.

??? note "Step 3 - Run Pre-upgrade Health Check *(optional)*"
    ```shell
    sudo ipf-checker -d -s
    ```
    Let the test run to completion (≈ 30 minutes).
    Save the output for future reference; we would appreciate if you shared this output with our team, if possible.
    Reach out to the team [here](https://ipfabric.atlassian.net/servicedesk/customer/portal/1).

??? note "Step 4 - Launch the `v7.5` Upgrade"
    1. Log in via SSH as an administrative user.
    2. Download and execute the upgrade package provided by IP Fabric.
    3. Monitor the console; the upgrade will:
    ...
    4. When the upgrade completes successfully, proceed to Step 5.

??? note "Step 5 - Post-upgrade Validation"
    1. Log in to the UI, ensure the dashboard loads without errors. If you see errors, reach out to our team.
    2. Navigate through a few historical snapshots to confirm data integrity (i.e. check your list of devices to confirm it matches what you expect).
    3. Launch a Discovery and verify that it finishes.
    4. Optionally run a quick API request or path lookup to confirm core functionality.
    5. Observe CPU, RAM usage to ensure the VM is not overloaded.
